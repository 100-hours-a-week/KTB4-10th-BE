...
화면 수가 아니라 반복 데이터, 독립 생명주기, 참조·이력과 중복 방지로 나눴다. 생성 요청은 실패해도 작업이 남지만 가이드북은 성공 결과만 존재하고, 주문 하나에도 결제 시도는 여러 번 생길 수 있다.

### 왜 상태를 VARCHAR로 저장하는가?

상태는 DB에 실제 저장한다. 코드에서는 Enum으로 안전하게 다루고 DB 네이티브 Enum의 변경 부담을 줄이려고 VARCHAR를 선택했다.

### 왜 작업과 가이드북이 별도인가?

최초 생성 실패에는 가이드북이 없지만 오류와 재시도 정보는 남아야 한다. 프론트는 작업 ID로 조회하고 성공 후에만 가이드북 ID를 받는다.

### 왜 잔액과 원장이 둘 다 필요한가?

잔액은 빠른 현재 조회, 원장은 변동 원인의 증명이다. 같은 트랜잭션으로 갱신하고 원장으로 잔액을 검산한다.

### 왜 일부만 JSONB인가?

AI 입력·스냅샷처럼 변화가 잦고 원문 보존이 필요한 값만 JSONB다. 검색·관계·제약 값은 일반 컬럼이어야 한다.

### 왜 인덱스를 모두 걸지 않는가?

인덱스는 읽기를 빠르게 하지만 쓰기와 공간 비용이 있다. 실제 API 조회 조건과 실행 계획을 근거로 최소한만 둔다.
