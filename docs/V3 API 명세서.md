...
> 구현 상태: 생성 요청·현재 취향 스냅샷 저장과 커밋 후 AI 접수 이벤트, AI 요청 변환 및 외부 `job_id` 저장을 구현했다. 현재 자동 트리거는 Fake AI Client가 활성화되는 `local` 프로필에서만 동작한다. 실제 AI HTTP Client, 생성권 검증·차감, 동시성 보강과 실패 복구를 마치기 전에는 프론트엔드 사용자 흐름에 연결하거나 운영에 공개하지 않는다.
